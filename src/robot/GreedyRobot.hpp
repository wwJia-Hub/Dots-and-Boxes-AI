...
class GreedyRobot : public Robot<BoardSize> {
  ...
  bool EnemyUnscoreable() const;
  bool Scoreable() const;
  Array<Edge<BoardSize>, Edge<BoardSize>::Max>& GetEdgeBuffer();
...
template <int64_t BoardSize>
bool GreedyRobot<BoardSize>::EnemyUnscoreable() const {
  return EnemyUnscoreableIndex < Edge<BoardSize>::Max;
}

template <int64_t BoardSize>
bool GreedyRobot<BoardSize>::Scoreable() const {
  return ScoreableIndex > 0;
}

template <int64_t BoardSize>
Array<Edge<BoardSize>, Edge<BoardSize>::Max>& GreedyRobot<BoardSize>::GetEdgeBuffer() {
  return Edges;
}
