...
class Square : public IntWapper<BoardSize> {
  ...
  constexpr Int<BoardSize> X() const;
  constexpr Int<BoardSize> Y() const;
};

template <int64_t BoardSize, int64_t Length>
constexpr Square<BoardSize, Length>::Square(Int<BoardSize> x, Int<BoardSize> y)
    : IntWapper<BoardSize>(x * Length + y) {}

template <int64_t BoardSize, int64_t Length>
constexpr Int<BoardSize> Square<BoardSize, Length>::X() const {
  return IntWapper<BoardSize>::v / Length;
}

template <int64_t BoardSize, int64_t Length>
constexpr Int<BoardSize> Square<BoardSize, Length>::Y() const {
  return IntWapper<BoardSize>::v % Length;
}
